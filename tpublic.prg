...
METHOD IsDef( cName )  CLASS TPUBLIC
   local oErr
   /* Se anexa esta modificacion para evitar caida del servidor
      por un error en este metodo, a veces ocurre mientras se
      hacen pruebas.  RIGC 2019-07-16 */
   TRY
      if VALTYPE(cName)!="C"
         cName := NIL
      endif
   CATCH oErr
      tracelog cName, oErr:description, hb_eol(), hb_valtoexp(oErr)
      return .F. 
   END
RETURN hb_hHasKey( ::hVars, cName )
